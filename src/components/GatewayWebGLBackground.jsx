...
export default function GatewayWebGLBackground({ activeDiscipline = null }) {
  ...
  useEffect(() => {
    ...
    // Deferred initialization to prioritize main-thread LCP paint
    const initWebGL = () => {
      if (!isMounted) return;
      ...
      // SPEED OPTIMIZATION: DPR capped at 1.0x for max rendering throughput
      ...
      let lastFrameTime = 0;
      const targetFPS = 30;
      const frameInterval = 1000 / targetFPS; // ~33.3ms throttle cap

      // Render Animation Loop with 30 FPS Cap
      const render = (now) => {
        if (!isMounted) return;
    ...
    // Defer initialization to free main-thread for LCP paint
    if ('requestIdleCallback' in window) {
      idleId = requestIdleCallback(() => initWebGL(), { timeout: 250 });
    } else {
      timeoutId = setTimeout(() => initWebGL(), 150);
    }
