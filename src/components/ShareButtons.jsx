...
export default function ShareButtons({ title = '', excerpt = '', isLight = false }) {
  ...
  const shareLinks = [
    ...
    {
      name: 'LinkedIn',
      titleFa: 'اشتراک‌گذاری در لینکدین',
      titleEn: 'Share on LinkedIn',
      icon: (
        <svg className="w-4 h-4 sm:w-4.5 sm:h-4.5" viewBox="0 0 24 24" fill="currentColor">
    ...
    {
      name: 'Facebook',
      titleFa: 'اشتراک‌گذاری در فیس‌بوک',
      titleEn: 'Share on Facebook',
      icon: (
        <svg className="w-4 h-4 sm:w-4.5 sm:h-4.5" viewBox="0 0 24 24" fill="currentColor">
    ...
    {
      name: 'Email',
      titleFa: 'ارسال با ایمیل',
      titleEn: 'Share via Email',
      icon: (
      ...
      url: `mailto:?subject=${shareTitle}&body=${shareExcerpt}%20${shareUrl}`,
    },
  ];
  ...
  return (
    <div className="py-6 my-0 border-t border-b border-current/10 w-full flex flex-col sm:flex-row items-center justify-between gap-4 md:gap-6">
      
      {/* Dynamic Bilingual Label */}
      <div className="flex items-center gap-2 text-xs sm:text-sm md:text-base font-semibold tracking-wider opacity-80">
        ...
        <span>{isFa ? 'اشتراک‌گذاری پروژه :' : 'SHARE ENTRY :'}</span>
      </div>

      {/* Buttons Container */}
      <div className="flex flex-wrap items-center justify-center sm:justify-end gap-2.5 sm:gap-3.5">
        ...
        <button
          ...
          className={`flex items-center gap-2 h-9 sm:h-10 px-3.5 sm:px-4 border text-xs sm:text-sm font-medium transition-all duration-200 ${
            isLight
        ...
        {shareLinks.map((link) => (
          <a
            ...
            title={isFa ? link.titleFa : link.titleEn}
            aria-label={isFa ? link.titleFa : link.titleEn}
            className={`flex items-center justify-center h-9 w-9 sm:h-10 sm:w-10 border transition-colors ${
              isLight
                ? 'border-black/20 hover:border-black hover:bg-black/5 text-black'
                : 'border-white/20 hover:border-white hover:bg-white/10 text-white'
            }`}
        ...
        {/* Native Mobile Share Sheet Fallback */}
        {typeof navigator !== 'undefined' && navigator.share && (
          <button
            onClick={handleNativeShare}
            type="button"
            title={isFa ? 'اشتراک سیستم' : 'Native Share'}
            aria-label={isFa ? 'اشتراک سیستم' : 'Native Share'}
            className={`flex items-center justify-center h-9 w-9 sm:h-10 sm:w-10 border transition-colors ${
              isLight
                ? 'border-black/20 hover:border-black hover:bg-black/5 text-black'
                : 'border-white/20 hover:border-white hover:bg-white/10 text-white'
